...
5. **Idle only when your Output reflects the current world.** `idle` is the only step that ends a cycle. A cycle that idles having produced nothing — or that spends its steps inspecting files instead of producing — has failed: when you have no Output yet, or the world has changed, write or refresh it before idling. When your Output is already current and nothing has changed, idling is correct.
